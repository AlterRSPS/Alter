...
class LeatherAction {
    ...
    private val leatherItemNames =
        Leathers.leatherDefinitions.flatMap { it.value.values }.distinct().associate {
            it.id to
                getItem(
                    it.id,
                ).name.lowercase()
    ...
    suspend fun leathers(
        task: QueueTask,
        leathers: Int,
        leatherItem: LeatherItem,
        amount: Int,
    ) {
    ...
    private suspend fun canLeather(
        task: QueueTask,
        leathers: Int,
        leatherItem: LeatherItem,
        ...
        if (inventory.getItemCount(leathers) < leatherItem.leatherCount) {
            if (sendMessageBox) {
                task.messageBox(
                    "You need ${leatherItem.leatherCount} ${leatherNames[leathers]} to make a ${leatherItemNames[leatherItem.id]}",
                )
            }
            return false
        }
            ...
            if (sendMessageBox) {
                task.messageBox(
                    "You need a ${Skills.getSkillName(
                        player.world,
                        Skills.CRAFTING,
                    )} level of at least ${leatherItem.level} to fletch ${leatherItemNames[leatherItem.id]}.",
                )
            }
            return false
        }
