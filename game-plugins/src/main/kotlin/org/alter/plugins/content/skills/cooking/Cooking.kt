...
class Cooking {
    ...
    suspend fun cook(
        ...
        repeat(amount) {
            ...
            } else if (level.interpolate(
                    minChance = 60,
                    maxChance = 190,
                    minLvl = food.minLevel,
                    maxLvl = food.maxLevel,
                    cap = 255,
                )
            ) /*|| player.getVarp(TutorialIsland.COMPLETION_VARP) == 90 || player.getVarp(TutorialIsland.COMPLETION_VARP) == 160)*/ {
    ...
    private fun canCook(
        ...
        if (player.getSkills().getCurrentLevel(Skills.COOKING) < food.minLevel && !forceBurn) {
            player.filterableMessage(
                "You need a ${Skills.getSkillName(player.world, Skills.COOKING)} level of at least ${food.minLevel} to cook $name.",
            )
            return false
        }
        return true
    ...
    private fun canCombine(
        ...
        if (player.getSkills().getCurrentLevel(Skills.COOKING) < ingredient.minLevel) {
            player.filterableMessage(
                "You must have a ${Skills.getSkillName(player.world, Skills.COOKING)} level of ${ingredient.minLevel} to make $name.",
            )
            return false
        }
