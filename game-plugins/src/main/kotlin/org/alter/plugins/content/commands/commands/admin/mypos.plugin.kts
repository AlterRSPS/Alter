...
arrayOf("mypos", "coords", "pos").forEach { pos ->
    ...
    on_command(pos, Privilege.ADMIN_POWER, description = "Your position in coordinates") {
        ...
        if (instancedMap == null) {
            player.message("Tile=[<col=801700>x: ${tile.x}, y: ${tile.y}, height: ${tile.height}</col>], Region=${player.tile.regionId}")
        } else {
            val delta = tile - instancedMap.area.bottomLeft
            player.message("Tile=[<col=801700>x: ${tile.x}, y: ${tile.y}, height: ${tile.height}</col>], Relative=[${delta.x}, ${delta.y}]")
        }
    }
}
