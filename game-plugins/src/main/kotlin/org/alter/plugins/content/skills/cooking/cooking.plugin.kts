    ...
    if (obj.isRange) {
        on_obj_option(obj.objId, "cook") {
            ...
            player.queue(TaskPriority.WEAK) {
                cookingMessageBox(*cookableFoods, title = "What would you like to cook?", obj = obj, logic = ::cookFoodOnRange)
            }
        }
    ...
    foods.forEach { food ->
        on_item_on_obj(obj.objId, food.raw_item) {
            ...
            if (player.inventory.getItemCount(food.raw_item) == 1) {
                ...
                player.queue(TaskPriority.WEAK) {
                    cookingMessageBox(*foods, title = "How many would you like to cook?", obj = obj, logic = ::cookFoodOverFire)
                }
            }
        }
...
ingredients.forEach { ingredient ->
    ...
    if (!ingredientListenHashes.containsKey(hash)) {
        on_item_on_item(ingredient.item1, ingredient.item2) {
            ...
            player.queue(
                TaskPriority.WEAK,
            ) { cookingMessageBox(*foods, title = "What do you wish to make?", obj = null, logic = ::combineIngredients) }
        }
...
fun findIngredientCombinations(
    player: Player,
    food: CookingIngredient,
): IntArray {
    val foods = IntArrayList()
    ingredients.forEach { ingredient ->
